<div class="tab-pane" id="banner" role="tabpanel">
    ...
    <div class="row">
        @foreach (App\Models\Backend\Setting::activeBanner() as $row)        
            <div class="col-lg-12">
                @switch($row->form)
                    @case('text')
                    ...
                    @case('file')
                        ...
                        @endphp
                        {!! Form::file('file_'.$row->name, $row->display_name)->attrs(['autocomplete'=>false,'class'=>$row->name])->help('Ukuran Gambar: '.$ukuran[$row->name]) !!}

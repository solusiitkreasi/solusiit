...
<div class="col-lg-12">    
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{route('admin.testimony.create')}}" class="btn btn-primary btn-sm float-right">
                <i class="fa fa-plus"></i>
            </a>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <x-data-tables-server-side :tableHeader="['No','Image','Nama','Status','Action']" id="data-table"></x-data-tables-server-side>
            </div>                
        </div>
    </div>
</div>    
...
<script>
    $(document).ready(function(e){
        ...
        const $table = $('#data-table').DataTable({
            ...
            ajax: {
                url : '{!! route("admin.testimony.source") !!}',
                dataSrc: 'data'
            },
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex',width:"2%", orderable : false, searchable:false, className: 'text-center'},
                {data: 'image', name: 'image',width:"2%", orderable : false, className: 'text-center'},
